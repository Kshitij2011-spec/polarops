  ...
  return (
    <div className="space-y-6 animate-fade-in pb-16">
      ...
      <div className="rounded-xl border border-polar-700 bg-polar-800/70 p-5 backdrop-blur-sm shadow-md space-y-4">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <span className="rounded bg-purple-950 px-2 py-0.5 text-xs font-mono font-bold text-purple-300 border border-purple-700">
          ...
          <div className="flex items-center gap-2 text-xs font-mono text-amber-300 bg-amber-950/40 border border-amber-800/80 rounded-lg p-2.5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 pt-3 border-t border-polar-700">
          {/* 1. Scenario Type */}
          <div className="space-y-1.5">
            <label htmlFor="scenario-type-select" className="text-xs font-mono text-polar-400">SCENARIO TYPE</label>
            <select
              id="scenario-type-select"
              value={scenarioType}
              onChange={(e) => setScenarioType(e.target.value)}
              className="w-full rounded-lg border border-polar-700 bg-polar-900 px-3 py-2 text-xs font-mono text-polar-200 focus:outline-none focus:border-accent-cyan"
            >
          ...
          {/* 2. Target Asset */}
          <div className="space-y-1.5">
            <label htmlFor="target-asset-select" className="text-xs font-mono text-polar-400">TARGET ASSET</label>
            <select
          ...
          {/* 3. Duration Selector */}
          <div className="space-y-1.5">
            <label className="text-xs font-mono text-polar-400">FAILURE DURATION</label>
                ...
                <button
                  ...
                  className={`flex-1 py-2 text-xs font-mono font-bold rounded-lg border transition-colors cursor-pointer ${
                    durationHours === d
                      ...
                      : "bg-polar-900 border-polar-700 text-polar-400 hover:text-polar-200 hover:border-polar-600"
                  }`}
                >
                  {d}h
                </button>
          ...
          <div className="flex flex-col justify-end">
            <button
              ...
              className="w-full flex items-center justify-center gap-2 rounded-lg bg-purple-600 hover:bg-purple-500 text-white px-4 py-2.5 text-xs font-mono font-bold shadow-md transition-all cursor-pointer disabled:opacity-50"
            >
      ...
      {result && (
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="rounded-xl border border-purple-800/80 bg-purple-950/20 p-5 space-y-3">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                <span className="px-2 py-0.5 rounded text-xs font-mono font-bold bg-purple-900 text-purple-200 border border-purple-700">
                  {result.scenario_id}
                </span>
                <span className="text-xs font-mono text-polar-300">
          ...
          <div className="rounded-xl border border-polar-700 bg-polar-800/70 p-5 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3 text-xs font-mono text-polar-400">
                ...
                <span className="flex items-center gap-1">
                  <span className="h-2 w-2 rounded-full bg-purple-400"></span> Scenario
                </span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3">
              {result.deltas.map((delta, idx) => (
                <div
                  key={idx}
                  className="rounded-lg border border-polar-700/80 bg-polar-900/80 p-3.5 space-y-2 flex flex-col justify-between"
                >
                  <div className="text-[11px] font-mono text-polar-400 truncate">
                    {delta.name}
                  </div>
          ...
          <div className="rounded-xl border border-polar-700 bg-polar-800/70 p-5 space-y-4">
            ...
            <div className="space-y-3">
              {result.affected_services.map((srv, idx) => (
                <div
                  key={idx}
                  className="rounded-lg border border-rose-800/60 bg-rose-950/20 p-4 space-y-2"
                >
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold font-mono text-polar-100">{srv.name}</span>
                      <span className="px-1.5 py-0.5 rounded text-[10px] font-mono bg-polar-800 text-polar-300 border border-polar-700">
                        {srv.code}
                      </span>
          ...
          <div className="rounded-xl border border-polar-700 bg-polar-800/70 p-5 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs font-mono text-polar-400">Human-in-the-loop Advisory</span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {result.decision_options.map((opt, idx) => (
                <div
                  key={idx}
                  className="rounded-lg border border-polar-700/80 bg-polar-900/70 p-4 space-y-3 flex flex-col justify-between"
                >
                  <div className="space-y-2">
                    ...
                    <div className="rounded bg-polar-800/60 p-2.5 text-[11px] font-mono text-polar-300">
